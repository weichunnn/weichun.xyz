...
For example, if the domain name is `weichuntan.com`, the current TLD server (which is responsible for `.xyz`) will return the IP address of the authoritative nameserver for `weichun`.
