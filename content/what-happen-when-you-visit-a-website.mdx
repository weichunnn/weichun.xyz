---
title: What happen when you visit a website?
...
For example, if the domain name is `example.com`, the current TLD server (which is responsible for .com) will return the IP address of the authoritative nameserver for `example`.
